...
ゲーム中に **`G` キー**を押すと「釣り調整パネル」が開きます（歩行サンドボックス・釣り画面のどちらでも）。
確率・秒数・メーターのゾーン・救済の効き方をスライダーで変えられ、変えた値はブラウザに保存されて
**パネルを閉じたあとの次のキャストから**効きます。開いている間はゲームが一時停止します。

- **初期値の置き場所**: `src/data/fishingTuning.ts` の `DEFAULT_FISHING_TUNING`
- パネルの「初期値に戻す」を押すと、いつでもこのファイルの値へ戻ります。
- 触って決まった値を**正式採用する**ときは、パネルの「設定をコピー」で JSON を書き出して
  初期値へ反映し、**このログにテイクとして追記する**。
  パネルで変えただけでは自分のブラウザにしか残らず、履歴も残りません。

数値の主な定義場所（旧プロト・320×180。作り直しで不要になる予定）:
